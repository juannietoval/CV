    ...
    <div className="relative min-h-screen overflow-hidden">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'py-2 md:py-3' : 'py-4 md:py-6'}`}>
        <div className="max-w-6xl mx-auto px-4 md:px-6">
          <div className="glass rounded-2xl md:rounded-full px-4 md:px-6 py-2 md:py-3 flex flex-col md:flex-row justify-between items-center bg-gradient-to-r from-indigo-100/70 via-white/80 to-blue-100/70 border-white/40 shadow-lg shadow-indigo-200/20 gap-3">
            
            {/* Logo o Nombre (Opcional, se puede omitir si el espacio es poco) */}
            <div className="hidden lg:block font-bold text-indigo-900 text-sm whitespace-nowrap">
              M.A. PUENTES CASTRO
            </div>

            {/* Menú Agrupado */}
            <div className="flex flex-wrap justify-center gap-2 sm:gap-4 md:gap-6 text-[10px] sm:text-[11px] md:text-xs font-bold uppercase tracking-wider">
              {/* Grupo Perfil */}
            ...
            <button 
              ...
              className="flex items-center gap-2 px-4 py-1.5 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 transition-all shadow-md hover:shadow-indigo-300/50 group"
            >
              <Search size={14} className="group-hover:scale-110 transition-transform" />
              <span className="text-xs font-bold hidden sm:inline">BUSCAR</span>
              <span className="text-[10px] opacity-60 hidden md:inline border border-white/30 px-1 rounded">⌘K</span>
            </button>
